...
public class ThamQuyenGiaiQuyetController extends BaseController<ThamQuyenGiaiQuyet> {
	...
	public ResponseEntity<ThamQuyenGiaiQuyet> create(@RequestBody ThamQuyenGiaiQuyet thamQuyenGiaiQuyet) {
		log.info("Tao moi ThamQuyenGiaiQuyet");

		repo.save(thamQuyenGiaiQuyet);
		return new ResponseEntity<>(thamQuyenGiaiQuyet, HttpStatus.CREATED);
	}

	@Autowired
	protected PagedResourcesAssembler assembler;
	...
	public @ResponseBody PagedResources<?> getList(Pageable pageable,
			...
			@RequestParam(value = "chaId", required = false) Long chaId) {
		log.info("Get danh sach ThamQuyenGiaiQuyet");

		Page<ThamQuyenGiaiQuyet> page = repo.findAll(thamQuyenGiaiQuyetService.predicateFindAll(ten, moTa, chaId), pageable);
		return assembler.toResource(page);
	}

//	@RequestMapping(method = RequestMethod.GET, value = "/chucvus/{id}")
//	public ResponseEntity getChucVu(@PathVariable("id") long id, PersistentEntityResourceAssembler assembler) {
...
//		}
//		return new ResponseEntity<>(assembler.toFullResource(cv), HttpStatus.OK);
//	}

	@RequestMapping(method = RequestMethod.GET, value = "/thamquyengiaiquyets/{id}")
	public ResponseEntity<ThamQuyenGiaiQuyet> getThamQuyenGiaiQuyet(@PathVariable("id") long id) {
		...
		return new ResponseEntity<>(thamQuyenGiaiQuyet, HttpStatus.OK);
	}

	@RequestMapping(method = RequestMethod.PATCH, value = "/thamquyengiaiquyets/{id}")
	public @ResponseBody ResponseEntity<ThamQuyenGiaiQuyet> update(@PathVariable("id") long id,
